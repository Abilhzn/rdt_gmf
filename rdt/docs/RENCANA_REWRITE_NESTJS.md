...
| 6a | Frontend — Fondasi (struktur folder, HTTP client, dev-shell proxy, shell/guards) | 🟨 Kode selesai, verifikasi UI ditunda ke Batch 8 |
| 6b | Frontend — Upload/Persist (repost-budgeting) | 🟨 Kode selesai, verifikasi UI ditunda ke Batch 8 |
| 6c | Frontend — Confirm (confirm/: confirmation+reassignment+investigation+comment) | 🟨 Kode selesai, verifikasi UI ditunda ke Batch 8 |
| 6d | Frontend — Dashboard + Dashboard-detail (home/, dashboard-detail/ nested) | 🟨 Kode selesai, verifikasi UI ditunda ke Batch 8 |
| 6e | Frontend — Export (need-approval/ + repost-history/) | 🟨 Kode selesai, verifikasi UI ditunda ke Batch 8 |
| 6f | Frontend — Admin + Setting-periode + Share-cost | 🟨 Kode selesai, verifikasi UI ditunda ke Batch 8 — **Batch 6 kode tuntas, TAPI belum diverifikasi jalan** |
| 7a | Finishing BACKEND (env fail-fast, `withTransaction` polish, lint 0/0, dead-code sweep, Swagger `@ApiTags`+v1.0.0) | ✅ Selesai (backend: build+lint 0/0+257 test hijau) |
| 7b | Finishing FRONTEND (dead-code konfirmasi bersih, `ng lint` 0/0 baru dipasang, test minimal split-sum/multi-value-filter) | ✅ Selesai (dev-shell: build+lint 0/0+5 suite/25 test hijau) — **verifikasi UI manual tetap nunggu Batch 8** |
| 8 | Verifikasi UI end-to-end (backend nyata + dev-shell nyata, klik manual semua flow) | ⬜ Belum — gerbang penutup sebelum handoff, gabung verifikasi Batch 6+7 sekali jalan |
...
Rewrite masuk **folder baru** (awalnya `rdt/backend-nest/`, biar backend Express lama yang masih
di `rdt/backend/` TIDAK ketimpa — dipakai sebagai referensi kebenaran & sumber logika/SQL yang
di-port. Express lama dihapus 27 Agu 2026 setelah rewrite tuntas & terverifikasi; folder rewrite
di-rename `rdt/backend-nest/` → `rdt/backend/` di titik itu, jadi satu-satunya backend di repo.)
...
**Koreksi (ditemukan saat grounding Batch 6):** `Login`/`SelectPlatform`/`CurrentUserService` sudah
sejak awal hidup di `AuthModule` TERPISAH (`rdt/frontend/../../auth/frontend/`), BUKAN di dalam
`RdtModule` — `RdtModule` cuma import+reroute ke situ. Jadi bukan RDT yang "membuang"
Login/SelectPlatform; keputusannya cuma soal apakah `RdtModule` masih meng-`import AuthModule`
saat integrasi OCX nanti (kemungkinan besar tidak, OCX punya identity flow sendiri) — itu satu
baris import, bukan restrukturisasi komponen. `IdentityService`/`RdtGuard` yang perlu disesuaikan
ke pola `IdentityProvider` backend, bukan komponen Login/SelectPlatform itu sendiri.
...
- Switch/if-else panjang atas status (`PENDING/EXCLUDED/NEEDS_REVIEW/NEEDS_INVESTIGATION/INVALID`; "Ask TA"=investigation; INVALID menang atas status lain termasuk EXCLUDED, lihat koreksi Batch 1) → kubur di level rendah,
...
### Batch 0 — Fondasi ✅
Scaffold NestJS di `rdt/backend/` + `core/` (config, DatabaseService+withTransaction,
...
- ⚠️ **Koreksi (ditemukan Batch 6b, 27 Agu):** ada status KELIMA yang kelewat didokumentasikan di sini — **`INVALID`** (`nominal`/kolom "In PCLC" gagal di-parse jadi angka). Di `resolveStatus()`, INVALID **menang atas status lain manapun termasuk EXCLUDED** (disengaja, sama seperti `excelParser.js` lama, ada di komentar kode). Fixture test Batch 1 kebetulan tak punya baris nominal-tak-valid jadi jalur ini tak pernah ke-exercise, tapi kode & CHECK constraint skema (`schema.sql`) sudah punya `INVALID` sejak awal. `RowStatus` yang benar: `PENDING | EXCLUDED | NEEDS_REVIEW | NEEDS_INVESTIGATION | INVALID` (+ `SPLIT_VOID` dari migration 012, dipakai share-cost 5.5b).
...
### Batch 6 — Frontend Angular [DIPECAH: 6a → 6b → 6c → 6d → 6e → 6f]
Sumber: `rdt/frontend/rdt/` (`RdtModule`, ~16 service + 8 modul fitur + shell/guards) — skala
sebanding gabungan Batch 3+4+5 backend, dipecah pakai pola sama (fondasi dulu, baru per-fitur).
**`rdt/frontend/dev-shell/`** cuma workspace CLI disposable buat `ng serve` lokal, BUKAN bagian
integrasi — disentuh seperlunya (update proxy target). `rdt/frontend/angular-integration-sample/`
adalah dokumentasi jalur integrasi manual buat tim IT, di luar scope kerja kode.

**Temuan penting saat grounding (koreksi §3):** `Login`/`SelectPlatform`/`CurrentUserService`
SUDAH hidup di `AuthModule` terpisah sejak awal, bukan di dalam `RdtModule` — lihat catatan §3.
`~16 service` di `rdt/frontend/rdt/services/` map hampir 1:1 ke module backend yang sudah jadi
(`confirmation.service`↔Batch3b, `dashboard(-detail).service`↔Batch5b/5c, `export-batch.service`↔
Batch4, `investigation`/`reassignment.service`↔Batch3c, `share-cost.service`↔Batch5.5b,
`notifications.service`↔Batch5a, dst) — remapping HTTP call ke rute backend jadi rendah-risiko
karena patokannya jelas. `dev-shell` proxy sekarang ke 3 backend service LAMA (port 4000/4001/4002)
— wajib diarahkan ke satu `backend`.

**6a — Fondasi:** struktur folder target (`core`/`shared`/`features` per §3), HTTP client ke SATU
backend (+ update proxy `dev-shell`), model/interface bersama, interceptor jembatan identity
(`CurrentUserService` shared tak diubah → field `id/dinas/role`-nya dipetakan ke header
`x-dev-user-id`/`x-dev-dinas`/`x-dev-role` yang dibutuhkan dev-mock `backend`), restrukturisasi
`shell`/`guards` (`RdtGuard`/`RoleGuard` — pindah lokasi + update komentar, logic tak berubah).

**6b — Upload/Persist** (`repost-budgeting` page) ↔ Batch 1 + 3.5b. ⚠️ **Komponen ini STALE** —
masih pakai `CONTRACT_FIELDS`/`GET /api/contract-fields` (format 53-kolom lama, sudah dibuang),
`TransactionStatus` masih include `INVALID` (harusnya `NEEDS_REVIEW`/`NEEDS_INVESTIGATION`),
response masih `{ok,rows,aggregation,error}` bukan `{data,message}`. Bukan restrukturisasi murni,
tapi nyambung ulang ke kontrak Format CBO.

**6c — Confirm** (`confirm/` SAJA — dikoreksi dari rencana awal "Confirm+Need-Approval") ↔
Batch 3b (confirm/decline queue) + 3c (reassignment DECLINED + investigation) + reuse service
comment-thread (5c). Satu komponen raksasa (import 6 service: `ConfirmationService`,
`ReassignmentService`, `InvestigationService`, `DashboardDetailService`, `DinasService`,
`TransactionService` stale) — cukup besar utk jadi batch sendiri.

**6d — Dashboard + Dashboard-detail** (`home/` + `dashboard-detail/`, dashboard-detail ternyata
**nested route DI DALAM `home.module.ts`**, bukan module terpisah — dikonfirmasi lewat baca kode)
↔ Batch 5b + 5c.

**6e — Export** (`need-approval/` + `repost-history/` — dikoreksi dari rencana awal, `need-approval/`
ternyata BUKAN reassignment/investigation, tapi flow TAB "waiting to repost": waiting queue +
transparency + confirm-batch-dengan-subdoc-pertama) ↔ Batch 4a + 4b + 4c.

**6f — Admin + Setting-periode + Share-cost** (tiga panel TAB-only self-contained, dikonfirmasi
...
seluruh `rdt/frontend/` (dicek langsung, bukan asumsi), dan belum ada laporan yang menjalankan
`backend` + `dev-shell` bersamaan lalu klik manual tiap flow di browser. "Field response
dikonfirmasi dari source" di tiap prompt 6b-6f berarti *dibaca*, bukan *dieksekusi*. Keputusan
(27 Agu): verifikasi UI ditunda sampai Batch 7 (finishing) juga selesai, digabung jadi satu
verifikasi Batch 8 — lihat bawah.

### Batch 7 — Finishing (BACKEND + FRONTEND)
Lint + hapus dead-code + Swagger final + checklist pre-merge; verifikasi 254 test backend hijau
end-to-end. **+ Env fail-fast** (schema validation `.env`, ditunda dari Batch 0). **+ Polish
`withTransaction`** (bungkus ROLLBACK try/catch + `release(err)` bila koneksi mati). **Scope
diperluas ke frontend** (definisi awal batch ini ditulis sebelum Batch 6 ada) — termasuk cek file
lama yang "dipertahankan sementara lalu dihapus begitu konsumen terakhir termigrasi" (6b-6f) sudah
beneran bersih semua, bukan ada sisa; dan pertimbangkan nambah SEDIKIT test otomatis (minimal
bisnis-logic kritis: SUM-validation share-cost, merge-vs-replace admin) mengingat frontend sekarang
nol test sama sekali.

**7a (backend) — tuntas 27 Agu:**
- `env.validation.ts` baru (`class-validator` `validateSync`, dipanggil lewat
  `ConfigModule.forRoot({ validate })`) — `DB_HOST/PORT/NAME/USER/PASSWORD` wajib ada (tak lagi
  fallback diam-diam di `configuration.ts`/`database.config.ts`); `IDENTITY_MODE`/`DIRECTORY_MODE`/
  `STORAGE_DRIVER` boleh kosong (fallback default) tapi kalau diisi typo → THROW; guard keras
  `NODE_ENV=production && IDENTITY_MODE!=='ocx'` → THROW saat boot (dibuktikan: exit code 1 +
  pesan jelas, lihat log boot). Boot normal terhadap `rdt_dev` tetap `{status:'ok',db:'ok'}`.
- `withTransaction` di `database.service.ts`: ROLLBACK dibungkus try/catch sendiri (gagal-nya
  di-log, tak menutupi error transaksi asli), `client.release(txnError)` supaya `pg` pool
  membuang koneksi yang berpotensi rusak alih-alih mendaur ulang. 3 test baru
  (`database.service.spec.ts`): happy-path, fn-throw-rollback-ok, rollback-gagal-sendiri.
- Lint sudah 0/0 dari sebelumnya (11 `eslint-disable` existing ditinjau, semua legit/beralasan
  komentar, dibiarkan). Dead-code sweep pakai `ts-prune`: 4 file genuinely dead dihapus
  (`base.repository.ts`, `pagination.dto.ts`, `status-konfirmasi.enum.ts`,
  `format-cbo-row.interface.ts` — nol import di manapun) + 1 re-export tak terpakai
  (`DirectoryEntry` di `mention-rules.ts`) dirapikan.
- Swagger: `@ApiTags` ditambah ke seluruh 14 controller (12 tag diminta + `health`), versi bump
  `0.1.0` → `1.0.0`, deskripsi disesuaikan (bukan lagi placeholder generik). Diverifikasi via
  `/docs-json` — semua 13 tag muncul terkelompok benar.
- Verifikasi akhir: `npm run build` bersih, `npm run lint` 0/0, `npm test` → **257/257 hijau**
  (254 lama + 3 baru), boot real lawan `rdt_dev` sukses, backend Express lama (waktu itu masih di
  `rdt/backend/`, sekarang sudah dihapus — lihat catatan folder rename di §2) tak disentuh.

**7b (frontend) — tuntas 27 Agu:**
- **Dead-code (klaim Batch 6b-6f) dikonfirmasi genuinely bersih, bukan re-klaim tanpa bukti:**
  grep atas semua nama file lama yang disebut laporan 6b-6f (`services/transaction.service.ts`,
  `services/export-batch.service.ts`, `services/confirmation.service.ts`, `services/admin.service.ts`,
  `services/share-cost.service.ts`, `services/investigation.service.ts`,
  `services/reassignment.service.ts`, `services/dashboard(-detail).service.ts`, dan folder lama
  `admin/`,`confirm/`,`home/`,`need-approval/`,`setting-periode/`,`share-cost/`,`repost-history/`,
  `dashboard-detail/`,`pages/repost-budgeting/`,`guards/`) → **nol hit** yang nyasar ke path lama;
  beberapa hit yang muncul di grep pertama (mis. `../services/confirmation.service` dari
  `features/confirmation/pages/confirm-page.component.ts`) diverifikasi resolve ke lokasi BARU
  (`features/confirmation/services/confirmation.service.ts`, file itu memang ada) — false positive
  dari pattern matching, bukan sisa lama. `rdt/frontend/rdt/` top-level dicek `ls`: cuma
  `core/`,`features/`,`services/`(4 file lintas-fitur: dinas/mention/modal/notifications),
  `shared/`,`shell/`,`assets/`+routing/module files — sesuai klaim laporan, tak ada folder fitur
  lama tersisa.
- **`ng lint`: sebelumnya TIDAK ADA sama sekali** (nol eslint config, nol `@angular-eslint/*` di
  `node_modules`, `ng lint` tak dikonfigurasi) — dipasang baru via `ng add @angular-eslint/schematics`
  (`eslint.config.js` baru + `angular.json`/`package.json` diupdate). **Ketemu bug saat setup:**
  `lintFilePatterns` default (`src/**/*.ts`) TIDAK mengikuti symlink `src/app/rdt` → `rdt/frontend/rdt`
  (dibuktikan: file dengan syntax error sengaja disisipkan ke `rdt/` tetap lolos "All files pass
  linting" sampai pattern ditambah eksplisit `src/app/rdt/**/*.ts`). Preset default
  `angular.configs.tsRecommended`+`templateRecommended`+`templateAccessibility` awalnya 363 lalu 237
  problem — mayoritas (`prefer-control-flow` 213×, `prefer-inject` 80×, `prefer-standalone` 27×) BUKAN
  bug, itu preferensi modernisasi standalone/`@if`/`inject()` yang kalau dipaksakan berarti nulis
  ulang tiap komponen di codebase NgModule-based ini — di-`off`-kan di config dengan komentar
  penjelasan, bukan "diperbaiki". Selector prefix diperlebar dari hanya `app` ke `['app','rdt']`
  (dua prefix nyata yang dipakai codebase — pages pakai `app-`, shared/reusable components pakai
  `rdt-`). Sisa 24 problem nyata diperbaiki manual: 3× `no-explicit-any` (cast test mock ke
  `CurrentUser` asli, `catch(err:any)`→`unknown`), 1× `array-type` (`Array<T>`→`T[]`), 2×
  `no-output-native` (`@Output() reset`/`cancel` collide dgn DOM event native → rename
  `resetRequested`/`cancelled`, update semua caller), 2× `no-empty-function` (CVA
  onChange/onTouched no-op stub — idiom Angular standar, `allow:['arrowFunctions']` di config, bukan
  "fix"), 6× `label-has-associated-control` (`rdt-mention-input` bungkus `<textarea>` asli tapi rule
  gak bisa lihat lewat boundary komponen → `controlComponents:['rdt-mention-input']`), 8×
  `click-events-have-key-events`+`interactive-supports-focus` (3 pasang murni bubbling-guard div,
  bukan kontrol interaktif — `eslint-disable` inline + komentar; 1 genuinely clickable — notif item
  di `shell.component.html` — diperbaiki nyata: `role="button" tabindex="0" (keydown.enter)`).
  **Hasil akhir: 0 warning, 0 error**, dan lint sekarang benar-benar meng-cover `rdt/`.
- **Test baru (3 file, semua hijau):**
  1. `features/share-cost/services/split-sum.util.spec.ts` (7 test) — mengunci `splitSumDiffCents`,
     fungsi murni yang diekstrak dari getter `sumDiff`/`sumMatches` di `split-form.component.ts`
     (diekstrak ke file terpisah tanpa import `@angular/core` supaya testable tanpa AOT-compile
     templat komponen). **Ketemu bug nyata saat menulis test ini** — implementasi lama
     (`Math.round((sumNominal - original) * 100) / 100`, sum float dulu baru dibulatkan) TIDAK
     presisi sama dengan validasi backend `share-cost.service.ts` (5.5b), yang membulatkan
     **tiap baris ke sen (integer cents) dulu baru dijumlah**: kasus split `[33.335, 33.335, 33.33]`
     vs original `100` — client lama bilang **valid**, tapi server hitung `3334+3334+3333=10001` sen
     vs `10000` sen → **SPLIT_SUM_MISMATCH**. Diperbaiki: `splitSumDiffCents` sekarang membulatkan
     tiap baris ke sen dulu, persis logic backend, sebelum ditest.
  2. `shared/multi-value-filter.component.spec.ts` (13 test) — mengunci `parseMultiValueFilter`,
     `matchesAnyFilterValue`, `matchesAllColumnFilters` (fungsi murni dipakai lintas beberapa fitur
     untuk filter kolom bergaya Excel-paste).
  3. Item "admin merge vs replace-all" (prioritas #2 di prompt) **dicek, TIDAK ADA logic state
     dibedakan** — `mapping-editor-page.component.ts` (merge) dan `exclusions-editor-page.component.ts`
     (replace-all) cuma beda teks hint UI + endpoint yang dipanggil; semantik merge/replace-all
     sepenuhnya di backend (di luar scope). Sengaja di-skip sesuai syarat prompt sendiri ("kalau ada
     logic UI...").
- Verifikasi akhir: `ng build` bersih (1 warning pre-existing tak berubah: budget scss
  `dashboard-page.component.scss` +816B — bukan dari batch ini), `ng lint` 0/0, `ng test` →
  **5 suite / 25 test hijau** (2 suite lama dari 6a + 3 suite baru). `rdt/backend*` dan
  `auth/frontend/` dikonfirmasi nol perubahan (`git diff --stat` kosong).
- **Belum selesai:** Batch 8 (verifikasi UI end-to-end manual di browser). Jangan tandai Batch 6/7
  penuh ✅ sebelum Batch 8 lolos.

### Batch 8 — Verifikasi UI end-to-end (gerbang penutup sebelum handoff)
Jalankan `backend` nyata (lawan `rdt_dev` atau DB test terpisah) + `dev-shell` nyata
(`ng serve`), klik manual tiap flow utama di browser sungguhan — mirror level rigor real-DB-
verification yang dipakai tiap batch backend, sekarang diterapkan ke frontend: upload→parse→
persist (6b), confirm/decline/reassign/investigation (6c), dashboard+drill-down+comment (6d),
waiting→confirm-batch→download→history (6e), admin mapping(merge)/exclusions(replace-all)
(6f), period-deadlines set+sweep+delete (6f), share-cost split+SUM-validation (6f). Cek juga
konsolidasi comment-thread 6c-vs-6d (6c sempat bikin versi lokal "tidak nunggu 6d" — pastikan
tak ada dua implementasi divergen). Baru boleh tandai Batch 6 & 7 penuh ✅ setelah ini lolos.
