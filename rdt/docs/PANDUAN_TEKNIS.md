...
| `rdt/backend` | 3000 | Otak bisnis — semua aturan (upload Excel, konfirmasi, reassign, dst) |
...
2. Kamu ketik username/password → Angular kirim **langsung** ke `auth` (port 4001), BUKAN lewat
   `rdt/backend` — beda dari backend Express lama (dihapus 27 Agu 2026), yang dulu jadi
   perantara login; `rdt/backend` (NestJS) sekarang gak ikut campur di proses login sama sekali
3. `auth` telepon `data_user` (port 4002): "orang ini beneran ada gak, dinas apa, role apa?"
4. Kalau cocok, `auth` kasih **token** (semacam gelang tangan konser — bukti kamu udah "masuk",
   dicek tiap kali kamu minta sesuatu), disimpen di browser kamu (`X-Session-Token`)
5. Tiap request Angular ke `rdt/backend`, sebuah "jembatan" di frontend
   (`IdentityBridgeInterceptor`) nempelin identitas dari langkah 4 itu jadi header
   `x-dev-user-id`/`x-dev-dinas`/`x-dev-role` — `rdt/backend` **percaya header itu langsung**
   (mode `IDENTITY_MODE=dev-mock`, dev-only), dia gak telepon balik ke `auth` buat verifikasi
   ulang
6. Nanti kalau RDT sudah beneran nempel ke OCX (lihat `IRS.md`), header identitas itu diisi OCX
   sendiri (`x-ocx-user-id`/`x-ocx-dinas`/`x-ocx-role`, `IDENTITY_MODE=ocx`) — flow login lewat
   `auth`/`data_user` di atas kemungkinan besar gak dipakai lagi di produksi
...
- **Role-based access** — PIC cuma bisa akses dinasnya sendiri, TAB akses lebih luas, dicek lewat Guard NestJS (`DinasAccessGuard`/`RolesGuard`) di `rdt/backend/src/core/security/` — dipasang per-controller pakai `@UseGuards(...)`, bukan middleware chain kayak Express lama
...
6. **`.env` di beberapa tempat berbeda** (`auth/`, `rdt/backend/`) — pastikan SEMUANYA ada di `.gitignore`, jangan cuma satu.
