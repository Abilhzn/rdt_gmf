import { naturalKeyOf, flagDuplicates } from './duplicate-check';
...
test('flagDuplicates downgrades a PENDING row to NEEDS_REVIEW when it matches a previously-persisted transaction (cross-upload duplicate)', () => {
  ...
  expect(out[0].status_konfirmasi).toBe(RowStatus.NEEDS_REVIEW);
...
test('flagDuplicates does NOT flag two legitimate same-document rows within a single new upload (no prior history)', () => {
  ...
  expect(out[0].status_konfirmasi).toBe(RowStatus.PENDING);
  expect(out[1].status_konfirmasi).toBe(RowStatus.PENDING);
});
...
test('flagDuplicates leaves EXCLUDED/INVALID/NEEDS_REVIEW rows untouched even if they match history', () => {
  const incoming = [
    {
      ...
      in_pclc: 100,
      dinas_target: 'TC',
      status_konfirmasi: RowStatus.EXCLUDED,
    },
    {
      document_no: 'D1',
      ref_doc: 'R1',
      ...
      in_pclc: 100,
      dinas_target: 'TC',
      status_konfirmasi: RowStatus.INVALID,
    },
  ];
  const existing = [
    {
  ...
  const out = flagDuplicates(incoming, existing);
  expect(out[0].status_konfirmasi).toBe(RowStatus.EXCLUDED);
  expect(out[1].status_konfirmasi).toBe(RowStatus.INVALID);
});
