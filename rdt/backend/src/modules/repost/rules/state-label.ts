...
export interface DeriveStateLabelInput {
  pendingCount: number;
  targetDinas: string;
...
export function deriveStateLabel({
  ...
  targetDinas,
  subdocNumbers,
}: DeriveStateLabelInput): string {
  if (pendingCount > 0) return `Waiting for confirmation ${targetDinas}`;
  if (subdocNumbers && subdocNumbers.length)
    return `Reposted by TAB with subdoc ${subdocNumbers.join(', ')}`;
