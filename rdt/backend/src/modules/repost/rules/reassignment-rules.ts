...
export const REASSIGN_CAP = 3;

export interface DinasCodeRow {
  code?: string | null;
}
...
export function buildValidCodeMap(rows: DinasCodeRow[]): Map<string, string> {
...
export interface ValidateReassignTargetInput {
  newTarget: unknown;
  validCodes: Map<string, string>;
  dinasInisiasi: string;
  currentDinasTarget: string;
  reassignCount: number;
}

export type ValidateReassignTargetResult =
  | { ok: true; newTargetUpper: string }
...
export function validateReassignTarget({
  ...
  currentDinasTarget,
  reassignCount,
}: ValidateReassignTargetInput): ValidateReassignTargetResult {
  if (reassignCount >= REASSIGN_CAP) {
    return {
      ok: false,
      httpStatus: 400,
      error: `reassign_count already at cap (${REASSIGN_CAP}) — this transaction must be resolved with action=BORNE`,
    };
  }
  if (!newTarget || typeof newTarget !== 'string') {
    return {
      ok: false,
      httpStatus: 400,
      error: 'new_dinas_target is required for action=REASSIGN',
    };
  }
  ...
  if (!matchedCode) {
    return {
      ok: false,
      httpStatus: 400,
      error: `new_dinas_target '${newTarget}' is not a known active dinas`,
    };
  }
  if (lookupKey === String(dinasInisiasi).toUpperCase()) {
    return {
      ok: false,
      httpStatus: 400,
      error: 'cannot reassign back to the original uploader dinas',
    };
  }
  if (lookupKey === String(currentDinasTarget).toUpperCase()) {
    return {
      ok: false,
      httpStatus: 400,
      error: 'cannot reassign to the dinas that just declined',
    };
  }
