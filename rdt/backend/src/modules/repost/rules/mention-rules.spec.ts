...
describe('extractMentionTokens', () => {
  test('extracts multiple distinct mentions from a message', () => {
    expect(extractMentionTokens('cc @demo-pic-tj dan @TC tolong cek')).toEqual([
      'demo-pic-tj',
      'TC',
    ]);
  });
...
describe('resolveMentionedUserIds', () => {
  test('resolves a direct user_id mention', () => {
    expect(resolveMentionedUserIds('cc @demo-pic-tj', directory)).toEqual([
      'demo-pic-tj',
    ]);
  });

  test('resolves a dinas-code mention to every user in that dinas', () => {
    expect(resolveMentionedUserIds('cc @TJ', directory)).toEqual([
      'demo-pic-tj',
    ]);
  });
  ...
  test('does not duplicate a user reachable via both a direct mention and their dinas', () => {
    expect(resolveMentionedUserIds('@demo-pic-tj @TJ', directory)).toEqual([
      'demo-pic-tj',
    ]);
  });
  ...
  // that briefly did the opposite has been removed, see mention-rules.ts).
  test("@TA resolves to dinas TA's own PIC, not TAB", () => {
    expect(resolveMentionedUserIds('cc @TA tolong cek', directory)).toEqual([
      'demo-ta',
    ]);
  });
...
// were named in the same shared text — see mention-rules.ts's header comment on this function.
describe('filterMentionsToPair', () => {
  ...
  test('keeps a mentioned user whose dinas IS part of the pair', () => {
    ...
    expect(filterMentionsToPair(mentioned, directory, ['TJ', 'TMM'])).toEqual([
      'demo-tmm',
    ]);
  });
  ...
  test('drops an id with no matching directory entry', () => {
    expect(
      filterMentionsToPair(['ghost-user'], directory, ['TJ', 'TA']),
    ).toEqual([]);
  });
});
