...
function validateFreeText(raw, { required = false, fieldLabel = 'Teks', maxLength = MAX_FREE_TEXT_LENGTH } = {}) {
  const trimmed = typeof raw === 'string' ? raw.trim() : '';
  if (!trimmed) {
    if (required) return { ok: false, error: `${fieldLabel} wajib diisi`, code: 'REQUIRED' };
    return { ok: true, value: null };
  }
  if (trimmed.length > maxLength) {
    return { ok: false, error: `${fieldLabel} terlalu panjang (maksimum ${maxLength} karakter, saat ini ${trimmed.length})`, code: 'TEXT_TOO_LONG' };
  }
  return { ok: true, value: trimmed };
}

module.exports = { validateFreeText, MAX_FREE_TEXT_LENGTH };
