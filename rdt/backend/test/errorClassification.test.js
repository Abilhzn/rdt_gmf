const { classifyError, CATEGORY } = require('../src/rules/errorClassification');

describe('classifyError (REQ-RDT-LEDGER-05 / REQ-RDT-AUDIT-02)', () => {
  test('Postgres lock/deadlock codes -> KONFLIK_KONKURENSI', () => {
    expect(classifyError({ code: '55P03' })).toBe(CATEGORY.CONCURRENCY_CONFLICT);
    expect(classifyError({ code: '40P01' })).toBe(CATEGORY.CONCURRENCY_CONFLICT);
    expect(classifyError({ code: '40001' })).toBe(CATEGORY.CONCURRENCY_CONFLICT);
  });
  ...
  test('network-level codes -> KONEKSI_TERPUTUS', () => {
    expect(classifyError({ code: 'ECONNREFUSED' })).toBe(CATEGORY.CONNECTION_LOST);
  ...
  test('message-based fallback for connection loss when no pg code present', () => {
    expect(classifyError(new Error('Connection terminated unexpectedly'))).toBe(CATEGORY.CONNECTION_LOST);
  });

  test('app-thrown validation errors (no pg code) -> DATA_TIDAK_VALID', () => {
    expect(classifyError(new Error('transaction not pending: 42'))).toBe(CATEGORY.INVALID_DATA);
    expect(classifyError(new Error('transaction target mismatch: 42'))).toBe(CATEGORY.INVALID_DATA);
  });

  test('unrecognized error -> LAINNYA', () => {
    expect(classifyError(new Error('something weird happened'))).toBe(CATEGORY.OTHER);
  });
