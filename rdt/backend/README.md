...
standalone `ui-demo.html` frontend was deleted 7 Agu, Angular is the only frontend now. See
`../CLAUDE.md` section 8.)**
...
`auth`'s `/verify`. Full local dev-shell setup: `../CLAUDE.md` section 5.
...
- `tools/backupDatabase.js` / `tools/restoreDatabase.js` — manual DB export/import, see
  `../docs/dump_db.md` and `../docs/CHECKLIST_LAUNCH.md` section 2.1 for the full story (why this
  exists instead of `supabase db dump` — that needs Docker, this doesn't).

## Details worth knowing
- **Non-negotiable business rules** (atomic ledger, confirm/decline semantics, audit trail,
  dinas-routing derivation) — `../CLAUDE.md` section 3. Read before touching anything in
...
- Full business/architecture context, current implementation status, and requirement docs:
  `../CLAUDE.md` (this app's own) and `../docs/SRS.md`.
